...
class ListProvider implements RemoteViewsService.RemoteViewsFactory {
    ...
    public RemoteViews getViewAt(int position) {
        ...
        rv.setTextViewText(R.id.stock_symbol1, symbol);
        rv.setTextViewText(R.id.bid_price1, bidPrice);
        rv.setTextViewText(R.id.change1, change);
        if (isUp == 1) {
            rv.setInt(R.id.change1, "setBackgroundResource", R.drawable.percent_change_pill_green);
        } else {
            rv.setInt(R.id.change1, "setBackgroundResource", R.drawable.percent_change_pill_red);
        }
